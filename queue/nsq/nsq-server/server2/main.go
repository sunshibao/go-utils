...
func (m *NsqConsumer) HandleMessage(msg *nsq.Message) (err error) {
	fmt.Printf("###testOneHour:%s,%s : recv from %v, msg:%v\n", time.Now(), m.Title, msg.NSQDAddress, string(msg.Body))
	return
}
...
func main() {
	...
	err := consumer.InitConsumer("Topic_Demo", "channel_first", "127.0.0.1:4161")
	if err != nil {
